...
acting. The previously-flagged `gates.js` require path was **verified correct on
2026-08-19** — `gates.js` line 5 really does `require('/home/claude/build/chessboard-bundle.js')`.
A separate, genuine error was found and corrected the same day: this document
had twice described the GeoCodec obfuscation hash as SHA3-512, which it is not
(see Constraint 3). If the doc and the code disagree, surface the conflict; the
...
|---|---|---|---|---|
| CDN: js-sha3 0.9.2, vexflow 4.2.3, leaflet, geographiclib-geodesic | vendored | `sha3_512` (frozen **grid-passphrase** shuffle only!), notation, map, geodesics | — | Pinned versions; js-sha3 is part of the frozen grid-passphrase pipeline. It is **not** used by GeoCodec obfuscation, which has its own non-FIPS-202 hash — see Constraint 3 |
...
| `js/lib/geosonify-codec-engine_v11_8a.js` | core | `GeoCodec`: `encodeHierarchical`/`decodeHierarchical` (lines 247/263), `tokenizeCode`, `applyObfuscation`, own **non-FIPS-202 Keccak variant** (misleadingly named `sha3_512_hex` — see Constraint 3), Karney/Vincenty distance | GeographicLib (optional) | **FROZEN** obfuscation derivation (see Constraints) |
...
| `js/lib/geosonify-passphrase-strength.js` | feature (advisory) | Passphrase entropy estimator, random generator, CJK variant diagnosis; reads the BIP39 grids as wordlists | `geosonify-grids-data.js` | **Touches no frozen format.** Node self-test: `node js/lib/geosonify-passphrase-strength.js --selftest` |
| `js/ui/geosonify-passphrase-ui.js` | ui (advisory) | Injects strength meter + generator into `#passphraseModal`; language picker, confirm-retype for CJK | strength module | Binds a **second** `input` listener rather than editing card-renderer's; "Use this" dispatches a synthetic `input` event |
...
- **Obfuscated code** (GeoCodec model): each non-final token's flat index shifted by distance-from-end mod N over a deterministic keyed shuffle seeded by the codec engine's **bespoke Keccak-variant hash (NOT SHA3-512 — see Constraint 3)** of `"0,1,…,N-1|lastIndexFlat"`; **final token unshifted** so the deepest cell stays addressable. The seed's only variable input is `lastIndexFlat`, which is public (it is the code's own final token), so obfuscation provides **no confidentiality** and adds no brute-force cost — an attacker precomputes all N shuffles once. Obfuscation destroys graceful truncation — documented trade-off [docs: `faq-data.js` ~lines 600–618].
...
- **CDN at load**: js-sha3 0.9.2 (`sha3_512`, used by the frozen **grid-passphrase** shuffle only — obfuscation uses the codec engine's own variant), VexFlow 4.2.3, Leaflet, geographiclib-geodesic (Karney; GeoCodec falls back to robust Vincenty if absent).
...
3. **FROZEN: GeoCodec obfuscation derivation** [code: `js/lib/geosonify-codec-engine_v11_8a.js` `buildIndexSeedString`/`generateStrongSeedFromIndices`/`generateShuffleOrderFromHash`; spec: `spec/FROZEN-GEOCODEC-OBFUSCATION.md` §1.0]: seed string format, hash, hash-extension loop, chunked-hex sort (3 hex chars per cell). Codes in the wild depend on it.
   **⚠ The hash is NOT SHA3-512, despite the function being named `sha3_512_hex`.** It deviates from FIPS 202 in two load-bearing ways: nonstandard ι round constants (a 38-entry table covering rounds 0–18; rounds 19–23 XOR zero), and a nonstandard sponge that absorbs each 32-bit message word into the **low half** of lane w only. θ/ρ/π/χ and the ρ offsets are standard. A re-implementer reaching for a stock SHA3-512 library produces wrong locations **with no error**. Blast radius is confined to this derivation — grid-passphrase (Constraint 1) and AES URL mode use genuine SHA3-512 / WebCrypto. Because obfuscation is not a security feature, nothing confidential rests on the hash being sound; it must be preserved for decodability, not for strength.
...
14. **Security honesty**: the passphrase permutation is bespoke, not cryptanalysed; anything needing real confidentiality must use AES mode. Don't market it otherwise [docs: `faq-data.js`, entries `obfuscation-vs-passphrase-vs-encryption` and `passphrase-brute-force`].
    Specifically: grid-passphrase v1 has **no key stretching and no salt**, both permanently (the derivation is frozen, so a work factor can never be raised). One candidate costs an attacker only N short hashes plus a sort — measured ~7,400 candidates/sec/core at N=36. An attacker who knows the rough region can reject a wrong guess after decoding a **single character** (≈1/36 survive), so precision beyond the region adds nothing to the attacker's cost. The unsalted fixed prefix means one precomputed table serves every user. The "a wrong guess just yields another valid location" property is also weaker than it sounds: targets are human-chosen, so ~71% of wrong decodes land in ocean and can be discarded on sight (~2 bits), and filtering to populated land is worth ~5 bits — probabilistic, stacks with regional knowledge, and does not break a strong passphrase, but it collapses a weak one's survivors to a shortlist someone can eyeball on a map. Practical floor: six random words (65.9 bits); seven if the region is guessable. Any change here must be a new versioned format, never a mutation of v1.
...
- Top-level let/const in index.html's inline scripts is invisible to every module — a top-level let binds in the global lexical environment, not on window, so global.currentCardCoord = … writes a different variable the inline script never reads. It always presents the same way: the module looks correct and does nothing, with no error. The two reliable bridges are inward: CardRenderer.setCoordinate(lat, lon), which fires onCoordChange and assigns the lexical binding [code: card-renderer.js ~5893; handler index.html ~1022]; and outward: publish onto window where the value is guaranteed current (window.__geosonifyLastSolution in updateCompactOutputs(), window.__geosonifyMap after MapManager.init). Reading window.someInlineVar directly gets undefined.
...
- **Grid arrays are global *lexical* bindings, not `window` properties.** `js/lib/geosonify-grids-data.js` declares every array with top-level `const` and never assigns to `window`. In a classic script that creates a binding in the global lexical environment, so `window.BIP39EnglishArray` is `undefined` while the bare identifier `BIP39EnglishArray` resolves fine from any later classic script. Code that probes `window[name]` to discover grids finds **nothing** and fails silently. Read the bare identifier (or `new Function`, if CSP allows). [code: `geosonify-grids-data.js`; verified 2026-08-19 in a real script-tag environment]
- **637 of the 2025 Japanese BIP39 entries are stored in NFD**, per the official BIP39 Japanese convention — e.g. あいだ is `あ+い+た+U+3099` (combining dakuten), not precomposed `だ`. Any code comparing user input (normally NFC) against these arrays must normalise both sides or the whole list silently fails to match. The arrays are FROZEN — normalise at the comparison, never rewrite the grid. Harmless for encoding: the frozen grid-passphrase derivation NFC-normalises the passphrase, and NFD/NFC forms verifiably derive identical permutations at N=36/256/2025. [code: `geosonify-grids-data.js`; verified against `spec/grid-passphrase-v1-reference.js`]
...
- **Node gate suites** (invariant tests, run manually — no CI config in the tree):
  ...
  - `js/lib/geosonify-passphrase-strength.js --selftest` — generation, rejection-sampling uniformity, weak-case rejection, repeated-word penalty, monotonicity. Advisory module; failure never affects encoding. [code]
...
- **Run the tests**: `node spec/grid-passphrase-v1-reference.js`; `node precision-gates.js`; `node gates.js` (fix its require path first); `node js/lib/geosonify-passphrase-strength.js --selftest`.
