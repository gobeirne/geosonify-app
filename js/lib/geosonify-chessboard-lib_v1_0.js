/*
 * geosonify-chessboard-lib_v1_0.js — Chessboard / HEALPix-Chessboard card vocabulary.
 * A Geosonify CODE PRESENTATION (not a coordinate encoder): hex code -> plausible legal-LOOKING
 * chess board, and board -> exact hex (leading zeros intact). One more alphabet, like ChromaCoord
 * for colour or Data Matrix for barcodes.
 * Engine: kings-out-of-rank (kings carry no payload; placed on canonically-first safe non-adjacent
 * empty pair) => boards are NEVER in check. Verified 0% check / total bijection / FEN+ASCII 20k /
...
ChessboardCodec.prototype.scan = function(board){
  ...
  const _codecs = {};
  function makeCodec(format){ const f=(format==='healpix')?'healpix':'standard';
    if(!_codecs[f]) _codecs[f]=new ChessboardCodec(CHESS_FAMILY,f); return _codecs[f]; }
  function maxHexDigits(format){ return makeCodec(format).maxHexDigits(); }
  function toBoard(hex,format){ return makeCodec(format).toBoard(hex); }
  function toFEN(hex,format){ return boardToFEN(makeCodec(format).toBoard(hex)); }
  function toASCII(hex,format){ return boardToASCII(makeCodec(format).toBoard(hex)); }
  function scan(input,format){ let board=input;
  ...
  function _coerce(input,format){ if(input instanceof Int8Array||Array.isArray(input))return input;
    if(typeof input==='string'){ if(/\|/.test(input)){const b=asciiToBoard(input);if(b)return b;}
      const f=fenToBoard(input); if(f)return f; return toBoard(input,format); }
    throw new Error('renderBoard: unrecognised input'); }
  function renderBoard(input,container,opts){ opts=opts||{};
    const board=_coerce(input,opts.format), letters=!!opts.letters, labels=opts.labels!==false;
    const FILES=['a','b','c','d','e','f','g','h'];
    const doc=container.ownerDocument||(typeof document!=='undefined'?document:null);
    container.innerHTML='';
    const wrap=doc.createElement('div'); wrap.className='gschess-boardgrid';
    if(labels){ const rl=doc.createElement('div'); rl.className='gschess-ranklab';
      for(let r=8;r>=1;r--){const s=doc.createElement('span');s.textContent=String(r);rl.appendChild(s);}
      wrap.appendChild(rl); }
    const bd=doc.createElement('div'); bd.className='gschess-board';
    for(let r=7;r>=0;r--)for(let f=0;f<8;f++){ const sq=doc.createElement('div');
      sq.className='gschess-sq '+(((r+f)&1)?'l':'d'); const c=board[r*8+f];
      if(c){const p=doc.createElement('span'); p.className='gschess-pc '+(c<=6?'w':'b');
        p.textContent=letters?LETTER[c]:SOLID[c]; sq.appendChild(p);} bd.appendChild(sq); }
    wrap.appendChild(bd);
    if(labels){ const fl=doc.createElement('div'); fl.className='gschess-filelab';
      for(const ch of FILES){const s=doc.createElement('span');s.textContent=ch;fl.appendChild(s);}
      wrap.appendChild(doc.createElement('div')); wrap.appendChild(fl); }
    container.appendChild(wrap); return board; }
  global.ChessboardLib={ makeCodec, maxHexDigits, toBoard, toFEN, toASCII, scan, renderBoard,
    fenToBoard, boardToFEN, asciiToBoard, boardToASCII, CHESS_FAMILY };
