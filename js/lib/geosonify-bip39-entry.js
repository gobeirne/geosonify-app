...
(function(global) {
  ...
  /*
    ...
    Every geographic hint here is Earth-specific: BIP39_GEO_LOOKUP comes from
    Natural Earth and IHO sea areas, and Nominatim is a street-address geocoder.
    In the celestial frame the SAME lat/lon numbers mean declination and right
    ascension, so these would confidently report "Tasman Sea" for a patch of
    Ophiuchus — plausible, and completely wrong.

    Suppressed rather than translated. The honest sky equivalent is an IAU
    constellation lookup and eventually a catalogue search; inventing a worse one
    meanwhile would be exactly the silently-wrong output this project avoids.
    Returning null just omits the hint — words and cells are unaffected.

    The Nominatim gate matters twice over: unsuppressed it would send celestial
    coordinates to a third-party Earth geocoder over the network.
  */
  function isSkyFrame() {
    try {
      if (typeof AppState !== 'undefined' && AppState.get) {
        const f = AppState.get('frame');
        if (f && f.sphere === 'sky') return true;
      }
    } catch (e) {}
    try {
      const w = (typeof window !== 'undefined') ? window : null;
      if (w && w.GeosonifySkyView && w.GeosonifySkyView.isOpen && w.GeosonifySkyView.isOpen()) return true;
    } catch (e) {}
    return false;
  }
  ...
  function getGeoDescriptorStatic(wordIndex) {
    if (isSkyFrame()) return null;                 // Earth hints only
  ...
  async function reverseGeocodeAddress(lat, lon) {
    if (isSkyFrame()) return null;                 // never geocode a sky position
  ...
  function updateSpeakerMode(gridKey) {
    ...
    const checksums = [];
    let concat = '';
    // (flat is already declared above; re-declaring it with const was a
    // SyntaxError that prevented this entire file from parsing.)
