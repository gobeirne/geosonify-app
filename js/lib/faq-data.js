      ...
      // ================================================================
      {
        ...
        items: [
...

          {
            ...
            a: `<p style="margin:0 0 12px;padding:8px 11px;border-radius:7px;background:var(--ios-light-gray,#f2f2f7);font-size:13px;"><strong>Beta:</strong> the Chessboard cards work and round-trip perfectly, but the board isn't fully shuffled yet - right now the bishops are sulking together in one corner instead of spreading out like the other pieces. We're fixing where they stand. Everything still encodes and decodes correctly in the meantime.</p>
...
<p>A naive version of this freezes the high-information squares - small codes leave the kings sitting on e1/g1 every time, which is the one tell a casual viewer catches. A <strong>whitening step</strong> spreads any code across the board's full capacity (a modular multiply by a stride derived from the golden ratio), so the king and bishop squares vary at the family's natural rate even for short codes. This step is itself a proven bijection, so nothing is lost.</p>
...
<p>Capacity follows from the material: a single 32-piece board of this family holds about <strong>78 bits</strong>, which is up to <strong>19 hex digits</strong>. The standard Chessboard card encodes to 18 hex (nine HexByte pairs); the HEALPix card to 19. That is far more than real coordinates need - a HEALPix cell at this depth is well under a tenth of a millimetre - so the limit is comfortable, and a code too precise to fit one board is <strong>refused, never truncated</strong>.</p>
<p>Reading a board back has one honest subtlety. The decoder recovers a code from <em>any</em> full board, but only boards the encoder actually produced are guaranteed to round-trip; a hand-edited or arbitrary legal position might decode to a code that re-encodes to a <em>different</em> board. So the scanner <strong>verifies on decode</strong>: after reading a code it re-encodes it and checks the result matches what was scanned, rejecting anything that doesn't as "not a valid card." This is why a doctored board - say, with a king nudged off its canonical safe square - is correctly refused rather than silently misread.</p>
