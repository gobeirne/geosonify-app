...

      {
        ...
        items: [

          {
            ...
            q: 'What is Geosonify?',
            a: `<p>Geosonify is an open framework for representing geographic locations in human-friendly ways. It was first developed by <a href="https://profiles.canterbury.ac.nz/Greg-O-Beirne">Greg O'Beirne</a> in 2012 and is released under the Mozilla Public License 2.0.</p>

<p>Most location systems give you one way to describe a place. Geosonify separates the geography from the representation. The same location can be expressed as letters and numbers, words and phrases, colours and visual patterns, musical notation, emoji, QR codes, or entirely custom vocabularies:
...
<p>The geography stays the same; only the representation changes.</p>`
          },
...
<p>Although we started with a 6x6 grid, it does not have to be this size. The same recursive subdivision works for any grid dimension, and Geosonify uses different grid sizes for different vocabularies - for example a 6×6 (36-cell) grid for the alphanumeric vocabulary, and a 45×45 (2025-cell) grid for the BIP39 word vocabularies. Each iteration selects exactly one cell, so the information added per character is log₂(grid size): for a 6×6 alphanumeric grid that is log₂(36) ≈ 5.17 bits per character, and an 8-character code over a 36-symbol grid corresponds to roughly 41 bits of positional search space.</p>
...
<p>The same prefix-sharing structure that makes alphanumeric codes compressible applies to word codes too - delta encoding operates on the token-level suffix of each code, so a NATO or BIP39 delta stream compresses by exactly the same logic as an alphanumeric one; the tokens are longer words but the prefix-sharing property is identical.</p>
...

          {
            ...
            a: `<p>Coordinates are sometimes communicated under difficult conditions. A word may be misheard, a character mistyped, or a location copied incorrectly from one system to another. Some Geosonify coordinate systems include a checksum that allows the recipient to verify that the coordinate arrived intact before attempting to use it. This is particularly useful when coordinates are relayed through multiple people or communicated in noisy environments.</p>`
          }
...
<p>For example, a random 2 m × 3 m spot inside Boston City Hall encodes as <code>7mn810bc2</code>. With obfuscation enabled, this becomes <code>f67jogqn2</code>. If someone attempts to decode that obfuscated coordinate as an ordinary hierarchical coordinate, they end up in a dry riverbed in Algeria.</p>

<p>Move only three metres east and the next coordinate becomes <code>7mn810bc3</code>, which obfuscates to <code>5kd5a5uv3</code>. Decoding that as an ordinary hierarchical coordinate places you on Bolshoy Lyakhovsky Island in the Siberian Arctic.</p>

<p>Passphrases create similar effects. Using the passphrase <strong>Cod</strong>, the original Boston location becomes <code>alknxyvbc</code>, which would normally place you beside the G552 National Highway in the Gobi Desert. Using <strong>Patriots</strong> produces <code>7dpi6zqle</code>, which would normally decode to a dirt track in Yellowstone County, Montana. Using <strong>Back Bay</strong> produces <code>9y1bcaq3m</code>, which would normally place you near the Tharthar Canal outside Tikrit, Iraq.</p>
