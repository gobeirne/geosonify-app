...

      {
        ...
        items: [

          {
            ...
            q: 'What is Geosonify?',
            a: `<p>Geosonify is an open framework for representing geographic locations in human-friendly ways. It was first developed by Greg O'Beirne in 2012, and is released under the Mozilla Public License 2.0.</p>
<p>Most location systems give you one way to describe a place. Geosonify separates the geography from the representation. The same location can be expressed as letters and numbers, words and phrases, colours and visual patterns, musical notation, emoji, QR codes, or entirely custom vocabularies. Each representation refers to the same underlying location.</p>
<ul>
...
<p>Where Geosonify differs is what that hierarchy is made of. In Plus Codes the hierarchy is expressed through a fixed alphanumeric code; in Geosonify the same hierarchical coordinate can be expressed as letters, words, colours, musical notes, QR codes, emoji, or an entirely custom vocabulary. Each representation retains the same hierarchical geography: words may suit radio communication, colour for a poster or printed tag, and music for a journey.</p>`
          },
...
<p>All of these represent the same underlying geography. They are alternative representations of the same location.</p>`
          }
      ...
      // ================================================================
      {
        ...
        items: [

          {
            ...
            a: `<p>ChromaCoord is a visual coordinate system. A location is encoded as hexadecimal values and rendered as a colour pattern. It resembles abstract artwork while remaining machine-readable: a Geosonify-compatible decoder can recover the original location from the image. Every location produces a distinct visual fingerprint, making ChromaCoord useful in contexts where a scannable image is more convenient than a string of characters, but you don't want the result to look like a binary QR-style code.</p>`
          },

          {
            id: 'chessboard',
            q: 'What are the Chessboard cards?',
            a: `<p>The Chessboard cards render a location as a <strong>chess position</strong> - a full board of 32 pieces that looks like a real game, but is actually a coordinate in disguise. Like ChromaCoord, it presents the coordinate in a familiar visual form while remaining fully machine-readable: a Geosonify decoder turns the board back into the exact same code it came from. There are two flavours, sharing identical board mechanics and differing only in which code they portray: <strong>Chessboard</strong> (a standard Geosonify hex code) and <strong>HEALPix &middot; Chessboard</strong> (a HEALPix hex code).</p>
<p>Every position the card draws is a <strong>legal-looking, settled diagram</strong>: both kings are present and neither is in check, so it passes a chess player's glance rather than reading as a glitch. The same code always produces the same board, and the board converts back to the code precisely - including any leading zeros, which are part of the code, not decoration. Each card offers its position as a <strong>FEN</strong> string (the standard text form chess software uses - the FEN label links straight to the lichess board editor) and as a copyable <strong>ASCII board</strong>, and either form can be pasted back in to recover the location. Codes finer than a single board can hold are refused rather than silently shortened.</p>

<details class="faq-details" style="margin-top:16px;border:1px solid var(--ios-separator,#c6c6c8);border-radius:8px;overflow:hidden;">
<summary class="faq-details-summary" style="cursor:pointer;padding:11px 14px;font-weight:600;font-size:14px;background:var(--ios-light-gray,#f2f2f7);list-style:none;display:flex;align-items:center;gap:8px;user-select:none;">&#9656;&nbsp;The details - how a coordinate becomes a legal chess board, and back - are here</summary>
<div class="faq-details-body" style="padding:2px 14px 6px;font-size:13.5px;line-height:1.55;">
<p>The card is a <strong>bijection between a hex string and a chess position</strong> - a reversible mapping where every code gives exactly one board and every valid board gives back exactly one code. It works in layers. First the hex string is packed into a single integer in a way that <strong>preserves length</strong>: the codes <code>96E</code>, <code>096E</code> and <code>0096E</code> are three different inputs and must yield three different boards, so the packing enumerates strings in (length, then value) order rather than collapsing to a bare number. That integer is then mapped to an arrangement of the 32 pieces using a mixed-radix <strong>ranking/unranking</strong> scheme: pawns, bishops, and the remaining material are each placed by an exact counting argument, so the integer and the board are two views of the same thing.</p>
<p>A naive version of this freezes the high-information squares - small codes leave the kings sitting on e1/g1 every time, which is the one tell a casual viewer catches. Two things prevent that. A <strong>whitening step</strong> spreads any code across the board's full capacity (a modular multiply by a coprime stride, inverted on decode), so even a short code reaches deep into the arrangement rather than hugging zero; and the layers are <strong>ordered by significance</strong> so that each piece group - including the bishops - is handed a genuinely varying digit rather than a starved one. Both steps are exact bijections, so nothing is lost, and every piece moves at the family's natural rate.</p>
<p>Keeping both kings out of check requires a separate step. Rather than filter out positions where a king is attacked (which would make the piece-counting depend on placement and break the exact capacity), the kings <strong>carry no information at all</strong>. The code determines every other piece; then the two kings are dropped onto the first pair of empty squares that are non-adjacent and unattacked, by a fixed rule the decoder can reverse by simply removing them. The kings carry no encoded data, so they can be placed deterministically on safe squares without affecting the round trip. (For full-material boards a safe pair always exists - measured at dozens of options per board, never zero.)</p>
<p>Capacity follows from the material: a single 32-piece board of this family holds about <strong>96 bits</strong>, which is up to <strong>23 hex digits</strong>. The standard Chessboard card encodes to 18 hex (nine HexByte pairs); the HEALPix card to 19. That is far more than real coordinates need - a HEALPix cell at this depth is well under a tenth of a millimetre - so the limit is comfortable, and a code too precise to fit one board is <strong>refused, never truncated</strong>.</p>
<p>When reading a board back, only positions produced by the encoder are guaranteed to round-trip, so a hand-edited or arbitrary legal position might decode to a code that re-encodes to a different board. So the scanner <strong>verifies on decode</strong>: after reading a code it re-encodes it and checks the result matches what was scanned, rejecting anything that doesn't as "not a valid card." The scanner therefore rejects a doctored board, such that a king nudged off its canonical safe square causes the board to be correctly refused rather than silently misread.</p>
<p style="margin-top:14px;padding-top:12px;border-top:1px solid var(--ios-separator,#c6c6c8);font-size:13px;color:var(--ios-secondary,#3c3c43);"><strong>Related work.</strong> Invertible ranking of chess positions - mapping integers to boards and back - is well-explored territory, most notably in John Tromp's <a href="https://github.com/tromp/ChessPositionRanking" target="_blank" rel="noopener">ChessPositionRanking</a>, which ranks legal chess positions and estimates how many exist, and which catalogues exactly the kinds of legality quirks (kings in check, adjacent kings, same-coloured bishops) that come up when you build something like this. Geosonify's Chessboard codec is a different, purpose-built system - it ranks one curated, fixed-material visible-board family to display and decode Geosonify hex strings, rather than ranking all legal positions - and does not use Tromp's code. We acknowledge his project as notable prior work in the field.</p>
</div>
</details>`
          },
...
<p>Under the notation, a rectangle is just three numbers: which way it is turned, how long it is, and how wide it is. Those three numbers can be written four different ways - as plain readable text, as two compact code styles, or as emoji - and all four describe exactly the same rectangle. All four notations describe identical geometry.</p>
...
<p>All four are losslessly interconvertible, because underneath they are the same centroid plus <code>(angle, L, S)</code>. Choose raw when readability matters, base36 for a compact number with no size ceiling, base64url for the shortest fixed code within the 1023 m limit, and emoji when you want it to be glanceable.</p>
...

          {
            ...
            a: `<p>A graticule is a box whose edges follow the lines of latitude and longitude - the parallels and meridians themselves - rather than being tilted at an arbitrary angle. It is defined by angular extent pinned to that grid: so many degrees of longitude wide, so many of latitude tall, around a centre point, so it needs no rotation angle. That is what sets it apart from a rotated rectangle, which is defined by metric side lengths that could sit anywhere on the globe. Map grid squares, administrative bounding boxes and many regional outlines are naturally graticules.</p>
<p>Because meridians converge toward the poles, a graticule is usually not a true rectangle on the ground: both side edges span the same degrees of longitude, but the pole-ward edge covers less distance than the equator-ward one, so the real footprint is a slight trapezoid. A full coordinate gives the centre point first - as a hierarchical code or as raw coordinates - then a double-tilde (<code>~~</code>), then the two spans. Those spans can be angles or a mix of angle and distance, whichever describes the box most naturally: Colorado is almost exactly <strong>7 degrees</strong> of longitude by <strong>4 degrees</strong> of latitude, written <code>~~7deg_4deg</code>, while South Australia pairs an angular width with a metric height as <code>t1fcio60c~~12deg_1341km</code>.</p>
...
<p>Unlike a rectangle, a graticule has no orientation angle. A rectangle stores orientation + long side + short side; a graticule stores width + height around a grid-aligned box.</p>
</div>
</details>`
          },

          {
            ...
            a: `<p>When you share a single location, the code is self-contained. But when you share a path or polygon - a route, a perimeter, a patrol area - you could have dozens or hundreds of points, each requiring a full code. Delta encoding is the compression scheme Geosonify uses to make multi-point shapes substantially more compact. For example, <a href="?b64dr=MqS9R69O~d4ZKPIZJa9ZIB1YX1s~d3W9WeMueSddXUdNYdasd_UlXOmqt~d4Zoi9ZxiVZ5FfhKUR~d3K_6T3wclHlGgnYjnzq~d4iwIAiwkiixwYi5BFi5idqBFL~d3JXMKqgK9xTp_bWvkbYk2htWQvYt~d4roAc~d3hYiZfeE8hNtSWOHWW1U1Vlgfmicnyj~d4zDnbzaBCyePFy2Mcy_Ud~d2rozWdb~d47JbZ7Juh7Liez7fAzh8ozS2izEqkr2TbrvTVsgdwsa0asjgrssjNs3E9txfzt-wYu8KT~d39JFvSfXYjXNjPkBWJUFVg~d4mdtomFlle936e-KQe203~d5-RhV_~d3xDzyJ4iDvPV9~d4LGqFLHpGMIHdMMQ7MMEzMFnoMHrYNFLDNeiKNu08VfwIdGv7dPZZde95e4Y2lHIflHzze43Aey1Le0Lie2JKf4BfnCdo~d3LHtNR4OR2~d5_gIJo~d4gCh7Y8Z1YuS8Rl2wRfa3SJ28SrdKaWjPjBdqrFM6j2Wsjv5ijvsjkgnjkaaZkTWtdwzRdWT2W52gWziyW0izecxAnRTr~d3R_ea_PsiX-af~d6T4oIIg~d3I-HaBOb0h~d4p555w_3n5DNX5FPuy55hy8qF6s-E~d6bACF4w~d3MnhVgpdAYcn0kPZsX71Ah9Jq~d4D6gpD9_jMAcrMCj4MMGhMOWjNKjv~d3Sm2TZxKm9Mt-~d6T4-Zsk~d4-R0a-BpOdjo4dafqdLYsdDZndDOEV9qWV-LaV3qeWpukWk2GXJ9mXKyRXLSiXD-3P92qP3um~d55Io7v~d3h_mjC9VwOPpd~d4JIGeJDWNJPwNaTU1as-SiF81iO41iW41i2PRi_IOqHB
nuGqulo2C182R~d45Ea6~d3E5ZMgpTuNT-qjeNj2VzKXzqX7WS~d5eBEhD~d3UsCdAylyztvJu6U29I-XN~d4KAcWKTxxK1K_SNTQSuQiSkxwShqAS4WFRcoORRrZQXSSQNW9I8xMIyY4IpQM~d5dPnRh~d3UwpTwsRn5QPY~d4OPHROGSVG8pEGqY3F8NDNKziNhYyM_ggUTMjUZH3TfuNTmNjT1APTyv8Tx2MS3GXS_7BanTGjKTCjKXwjE3fjGDLjHUXc4ir~d3ypezgbr1wf5XnqMvVn~d4dw5GlJxxlhsttx5Ct4bN0XeS0f8D02wM09eC8FNV~d3d7ksDb09e~d5lElbZlMELI~d3B61JaAJyyy2J7wg~d4UDYjULoQUaGncapHcxNecwEQcwjFcxtUkJl0kZk-kpy1k4m_sBPosL9usjxw0SVv0Sj28CsN8c6F8lV2~d5tFD6b~d4FVQPFVmDGpKgG95NOE5UOTpFVOZ6VrRJdISacXpKyXuoye97ymaIywSsywpi6Ixi5eh65mQo5uyM~d51BCH8~d4BCmABSFZJA8eJQ1JJgvIJpY0JwnwQPxOQXJ_QXrGQX1XRQhy~d3Jk6CxaCyACyfLLbjSzjeOko_~d4ZDVcZTA_ZayUZpG8hmiEhvJLiw_MqAvRpO8ppl7Apy2CwfSG~d3nnj3vp_bG_Q81lv16K~d44Eor~d3LzuroqyNnyzn~d59ACzw~d3B_HJE-In5Qk_YI6~d58HeaE~d3dIOKeMB-TIQz~d4GWGnGlACGlwXGk0fGkVUGJp6NGUgNHw-OIaR~d3JDjDQTDqULScSfNamMi_1zlg8OF8X1~d4WHQBXJwW~d3ROMSLrTxNbKwbawb5wiPuiWYirkqSJqxl6dD61z~d4fCE-fChyfKMIX0t3~d59YJJ1~d3RRwSlPTu4TedTKRLp4CwfBlBBfY~d4Q6i7~d36LYyyTxDfY8KT5dT9XjFKzH07uP~d4YGzHYGnIYPDjYH_hZAxSZAalRw6H~d3oUhgbsYyKYjqYmSiJAykZzUo7tk69O">a Base64 delta-encoding of the 862-point Berlin Wall polygon</a> is 73.3% smaller than the <a href="https://commons.wikimedia.org/w/index.php?title=Data:Berlin_Wall.map&action=raw">5-decimal-place raw coordinates</a> while improving coordinate quantization from approximately 0.68 m × 1.11 m to 0.18 m × 0.15 m.</p>
<p>It's a simple idea: Nearby points share long prefixes, so Geosonify transmits the first code in full and then only the part of each subsequent code that changes.</p>
...
<p>There are three main ways to say what you want - by name, by Wikidata entity, or by an exact OpenStreetMap ID (you can also load a GeoJSON, KML or GPX file) - plus an optional automatic mode that produces a finished, shareable code from any of them in a single link.</p>
...
<p>The importer also handles the largest meaningful ring in a multipolygon, merges fragmented ways into continuous paths, sorts ambiguous matches, and scales simplification to the size of the region. And because every stage is expressible as a URL parameter, the whole "name in, shape code out" pipeline fits in a single shareable link.</p>
...
<table>
  ...
  <tr><td><code>enc_pass</code></td><td>passphrase</td><td>Hard AES encryption (<code>?enc=</code> output)</td></tr>
			...
			
          {
            ...
            a: `<p>Yes. The <strong>🏠 Parcel</strong> source in Shape Import takes a street address or a raw <code>lat, lon</code> pair and fetches the legal cadastral parcel polygon returned by the relevant official service. It becomes an ordinary Geosonify shape: simplify it, encode it in any vocabulary, share it as a link.</p>
...
<p>Cadastral agencies caution that these polygons are <em>indicative</em> records of where the boundaries lie, not survey-accurate legal definitions. They are suitable for mapping, art and sonification, but should not be used in place of a cadastral survey.</p>
...
<p>All the seeds share the one format declared in the suffix. This is built for embedded real-estate and portfolio maps, so partial failures are reported explicitly: if some titles load and others don't, you get the ones that resolved plus a clear "<em>n of m titles loaded</em>" so the displayed map is not mistaken for a complete result. Each parcel is resolved independently, so one bad seed never sinks the whole import.</p>
...
<p>Obfuscation is reversible and requires no secret key. Anyone with Geosonify can decode an obfuscated code immediately. It hides visible patterns but provides no security or confidentiality.</p>
...
<div class="faq-details-body" style="padding:2px 14px 6px;font-size:13.5px;line-height:1.55;">
<p>Obfuscation is a reversible visual scrambling layer and provides no cryptographic protection. Anyone with the app can reverse it without a shared secret. Its purpose is to remove the hierarchical pattern that makes a plain code visually guessable, without requiring a shared secret.</p>
...
<ol>
  ...
  <li><strong>A deterministic hash-derived shuffle is generated from that seed.</strong> The hash output is expanded deterministically (by repeatedly re-hashing the whole accumulated string) until there is enough material to permute all N cells of the grid, which are then sorted into a new order. <strong>Note:</strong> this layer does <em>not</em> use SHA3-512, despite the function in the codec engine being named as though it does. It uses a bespoke Keccak-shaped hash that deviates from FIPS 202 in two ways (nonstandard round constants, and a sponge that only fills the low half of each lane). This is a historical quirk, now frozen: the derivation cannot be corrected without silently relocating every obfuscated code ever made. It is harmless here precisely because obfuscation provides no security at all — nothing rests on the hash being sound. Grid-passphrase mode and AES mode are separate code paths and use genuine, standard primitives. A re-implementer must port the bespoke hash exactly; reaching for a stock SHA3-512 library produces wrong locations with no error.</li>
...
<p>Obfuscation hides prefix patterns but prevents codes from being safely truncated.</p>
...
<p>For example, a random 2 m × 3 m spot inside Hut 8 at Bletchley Park encodes as <code>8bnljtbu9</code>. With obfuscation enabled, this becomes <code>7i737y789</code>. If someone attempts to decode that obfuscated coordinate as an ordinary hierarchical coordinate, they end up in rural Oregon near the Idaho border. Move only three metres west and the next coordinate becomes <code>8bnljtbu8</code>, which obfuscates to <code>imjd7ga58</code>, ordinarily floating off the Marquesas Islands in the middle of the Pacific.</p>
<p>Passphrases create similar effects. Using the passphrase <strong>RejewskiBomba</strong>, the original Hut 8 location becomes <code>5y9imm71w</code>, which would normally place you in far eastern Siberia. Using <strong>TuringBanburismus</strong> produces <code>friptx6b0</code>, which would normally decode to rural South Sudan.</p>
<p>People using the same passphrase decode both of these to Hut 8. People using a different passphrase decode them to somewhere else entirely. The passphrase selects a different symbol-to-location mapping, so the same nine characters name a different point for each passphrase.</p>
...
<div class="faq-details-body" style="padding:2px 14px 6px;font-size:13.5px;line-height:1.55;">
<p>When you add a passphrase, the symbol-to-cell mapping becomes dependent on it. The passphrase is first Unicode-normalised (NFC), so canonically equivalent Unicode representations of a passphrase — such as accented text or Māori macrons written as precomposed characters or as base-plus-combining-mark — behave identically on any device or operating system. At each encoding iteration the grid is permuted from a fixed domain-separation prefix, the normalised passphrase, and the sequence of cell indices chosen so far (the "chain"): each cell is assigned an independent <strong>SHA3-512</strong> digest computed over that preimage plus the cell index, and cells are sorted by those digests to give a keyed permutation. The chaining couples each iteration to previous choices, so symbols can't be peeled off and decoded one at a time.</p>
...
<p>Grid-passphrase v1 has <strong>no key-stretching</strong>, deliberately and permanently: its derivation is frozen so existing v1 codes stay byte-for-byte reproducible. A future versioned format could introduce stretching, but the work factor of existing v1 codes cannot be raised retroactively. Testing one candidate is nevertheless cheap: for a 36-cell grid each iteration needs only a small batch of SHA3-512 hashes and a tiny sort, with no deliberately expensive password KDF. By comparison, every guess against the AES URL layer costs 600,000 PBKDF2-HMAC-SHA256 iterations by design — orders of magnitude more expensive per guess. The exact ratio depends on hardware and implementation, but the direction is not in doubt: grid mode makes guessing cheap, AES mode makes it deliberately costly.</p>
...
<h4>Encryption</h4>
<p>Encrypts the entire coordinate payload using AES-256-GCM. Routes, shapes, metadata and coordinates are all protected. This hides not only the location values but the structure of what is being communicated — the grid type, the encoding, and the coordinate data itself. Payload length is padded to a 32-byte band, which removes fine-grained detail but does not fully conceal size: a long route still produces a visibly larger blob than a single point.</p>
...
Grid passphrase → a keyed private coordinate language; protects the coordinate value, not metadata; strength = passphrase strength.<br>
...
<p>Without a passphrase, Geosonify codes are <em>not</em> secret — an open, deterministic mapping from coordinates to symbols that anyone with the app can decode. Think of it like a grid reference: a notation system, not a cipher. Use it for compact, readable, grid-agnostic codes you don't mind anyone decoding.</p>
<p><strong>2. Grid passphrase (a keyed coordinate language)</strong></p>
<p>Adding a passphrase makes the mapping depend on it. The passphrase is Unicode-normalised (NFC) so it behaves identically across devices, then combined with a fixed domain-separation prefix and the chain of cell indices chosen so far; each cell gets an independent <strong>SHA3-512</strong> digest over that preimage plus the cell index, and cells are sorted by those digests to form a keyed permutation. The chaining stops symbols being decoded independently. It is deliberately simple — no stretching, salt, or authentication — because the format must stay byte-reproducible for decades.</p>
<p><em>What it does not do:</em> it does not hide metadata (a code's existence, length and therefore approximate precision, and often its grid vocabulary remain visible, and identical locations produce identical codes); it is not authenticated; and it cannot protect a weak passphrase. A strong, high-entropy passphrase is the whole of its security. It protects the coordinate value from people outside the passphrase group — not the fact that a location is being communicated.</p>
...
<ul>
  <li>Open DevTools → Sources and read every function. The key ones are <code>shuffleGridAndOrder</code> in <code>card-renderer.js</code> (grid passphrase) and <code>deriveEncryptionKey</code> / <code>encryptQueryString</code> in <code>index.html</code> (AES). They are unminified and commented.</li>
  <li>Watch the Network tab to confirm no requests during encode/decode.</li>
...
<ul>
  ...
  <li><strong>Intended threat model.</strong> Geosonify protects against casual observation, accidental disclosure, and offline interception of shared codes — not a compromised device or browser, a stolen or weak passphrase, or the unaudited bespoke grid mode used for anything high-stakes.</li>
  ...
  <li><strong>Determinism.</strong> For a given grid, precision and passphrase, the same location always produces the same code, so repeated encodings of one location are linkable to each other.</li>
  ...
  <li><strong>Long-term format compatibility</strong> is preserved by permanently freezing the grid-passphrase derivation and the base grids, so a faithful implementation of the frozen specification will keep decoding today's codes; see the frozen-format specification.</li>
</ul>
<table>
...
<p>For most cases where you just don't want a casual observer to read the location, the grid passphrase is sufficient. For high-stakes use where the location and the payload's internal structure should be confidential, use AES URL encryption with a strong passphrase. For public, shareable codes, use no passphrase.</p>
</div>
</details>`
          },
...
<p>The URL encryption layer hides the coordinates, grid type, encoding and other structure. Length is blurred rather than hidden — see the padding note below. It uses:</p>
<ul>
  <li><strong>PBKDF2</strong> with HMAC-SHA-256, <strong>600,000</strong> iterations, and a fresh random 16-byte salt per encryption. Each guess costs 600,000 iterations, and the per-message salt forces an attacker to repeat that work for every blob.</li>
  <li><strong>AES-256-GCM</strong> authenticated encryption: a wrong passphrase produces a detectable authentication failure rather than plausible garbage, so there is no "getting warmer" oracle.</li>
  <li>A random 12-byte IV per encryption, so identical plaintexts encrypt differently.</li>
  <li>A <strong>self-describing, versioned blob</strong>: a format-version byte and the iteration count are stored inside the blob, and the whole header (version, iterations, salt, IV) is bound as GCM additional authenticated data — tamper-evident, and the iteration count can be raised later without breaking existing links.</li>
  <li>Payload padding up to a 32-byte multiple, which blurs plaintext length to a 32-byte band.</li>
</ul>
<p>Blob layout: <code>BASE64URL( version[1] | iterations[3] | salt[16] | iv[12] | ciphertext+tag )</code>, emitted as <code>?enc=…</code>. AES-256-GCM with a salted PBKDF2-derived key is a standard construction with no known practical attacks when correctly implemented. The key-derivation function is <code>deriveEncryptionKey</code> and the entry point is <code>encryptQueryString</code>, both in <code>index.html</code>; AES-GCM and PBKDF2 use the browser's native <code>crypto.subtle</code> (WebCrypto) rather than a JavaScript implementation.</p>
</div>
</details>`
          }
...
<p>In 2021, a GoPro timelapse of a bicycle commute prompted the question of whether geographic movement could be converted into sound. To make this possible, Geosonify needed a musical grid where neighbouring cells were deliberately designed to differ by musically coherent intervals, so that small geographic movements would produce musically related changes rather than arbitrary jumps. This became the Geosonify sonification engine.</p>`
          },
    ...
    examples: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Berlin Wall',               href: '?b64dr=MqS9R69O~d4ZKPQZJa0ZIB1YX1r~d3W9eeMleSldXUdNYdasd_UlXOmqt~d4Zoi-ZxiUZ5FehKUY~d3K_7T3wclHlGgnYknzr~d4iwIAiwkaiw3Xi5BNi5idqBFT~d3JXTKpvK9wTp2bWnkbRk2gtWJvYu~d4roAV~d3hYqZffE8ZNtSWPAWWtU1dlgXmicnyb~d4zDnazS55yePNy2Mcy_UV~d2rozfdb~d47JbZ7Jug7Limz7eHzh7vzS2hzEqkr2TarvTVsgdwsa0asjgrssjNs3E9txfzt-wQu8KT~d39B9vTYXYjXNcPkJWJUFUn~d4mdtomFlle936e-JXe202~d5-RhV_~d3xDzyJ4iDmPV9~d4LGqFLHpGMIHWMMQ8MMEzMFnhMHrZ