...
<p>The <strong>never-in-check</strong> guarantee is the subtle part. Rather than filter out positions where a king is attacked (which would make the piece-counting depend on placement and break the exact capacity), the kings <strong>carry no information at all</strong>. The code determines every other piece; then the two kings are dropped onto the first pair of empty squares that are non-adjacent and unattacked, by a fixed rule the decoder can reverse by simply removing them. Because the kings are a deterministic decoration rather than data, the position is always legal-looking and the round-trip stays exact. (For full-material boards a safe pair always exists - measured at dozens of options per board, never zero.)</p>
...
<p>Reading a board back has one subtlety to be mindful of: The decoder recovers a code from <em>any</em> full board, but only boards the encoder actually produced are guaranteed to round-trip; a hand-edited or arbitrary legal position might decode to a code that re-encodes to a <em>different</em> board. So the scanner <strong>verifies on decode</strong>: after reading a code it re-encodes it and checks the result matches what was scanned, rejecting anything that doesn't as "not a valid card." This is why a doctored board - say, with a king nudged off its canonical safe square - is correctly refused rather than silently misread.</p>
...
<p>People using the same passphrase decode both of these to Hut 8. People using a different passphrase decode them to somewhere else entirely. The passphrase doesn't encrypt the location; it selects which of an astronomical number of coordinate languages you're speaking, so the same nine characters name a different point on Earth for every passphrase.</p>
