...
<p>It works in two steps: the address is geocoded to a point (using the same OpenStreetMap geocoder as place search), or you can import directly from the pin location, then the official cadastral service for that country is asked <em>"which parcel contains this point?"</em>. If several parcels sit at that point - cross-leases, unit titles, an ambiguous address - a picker lets you choose. You can also share the whole thing as <a href="https://geosonify.org/?address=-43.576504,%20172.674983&auto=1&grid=a&result=s">a single link</a> with <code>?address=...</code>, including the full automatic pipeline (<code>&amp;auto=1&amp;grid=a&amp;result=s</code> and friends, exactly as for <code>?place=</code>).</p>
